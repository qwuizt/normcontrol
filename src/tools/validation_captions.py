...
def _validate_start(context: CaptionValidationContext, rule: CaptionStartRule) -> bool:
    caption = context.caption
    logger = context.logger
    caption_text = _caption_words(caption)
...
def _validate_title_required(context: CaptionValidationContext, rule: CaptionTitleRequiredRule) -> bool:
    ...
    logger = context.logger
    caption_text = _caption_words(caption)
    start_kind = _table_caption_start_kind(caption_text, rule.similarity_cutoff)
    is_valid = True
...
def _validate_end_punctuation(context: CaptionValidationContext, rule: CaptionEndPunctuationRule) -> bool:
    ...
    start_kind = _table_caption_start_kind(caption_text, 0.7)
...
def validate_figure_section_number(
    ...
    int_section_number = _to_int(section_number)
...
def validate_figure_number_order(
    ...
    if not captions:
        return _to_int(caption.number) == 1

    prev_caption = captions[-1]
    current_number = _to_int(caption.number)
    previous_number = _to_int(prev_caption.number)
...
def _figure_caption_numbering_style_is_consistent(
    ...
    if _to_letter_index(caption.section_number) is not None:
        return True
...
def _figure_caption_section_order_is_valid(
    ...
    return (
        ...
        or _starts_letter_section_after_numeric_or_plain_numbering(caption.section_number, prev_caption.section_number)
    )


def _is_next_numeric_section(current_section: str | None, previous_section: str | None) -> bool:
    current_section_num = _to_int(current_section)
    previous_section_num = _to_int(previous_section)
    return (
...
def _is_next_letter_section(current_section: str | None, previous_section: str | None) -> bool:
    current_letter_index = _to_letter_index(current_section)
    previous_letter_index = _to_letter_index(previous_section)
    return (
...
def _starts_letter_section_after_numeric_or_plain_numbering(
    current_section: str | None,
    previous_section: str | None,
) -> bool:
    return _to_letter_index(current_section) is not None and not (
        _to_letter_index(previous_section) or _to_int(previous_section)
    )
...
def validate_figure_caption_section(
    ...
    if _is_latin_or_cyrillic_a(caption.section_number) and _is_latin_or_cyrillic_a(current_section_number):
        logger.warning(
...
def _validate_number(context: CaptionValidationContext, rule: CaptionNumberRule) -> bool:
    ...
    if not rule.section_allowed and context.caption.section_number is not None:
        if context.logger:
            context.logger.error(
                'нумерация по разделам запрещена критериями проверки',
    ...
    if rule.suffix_forbidden:
        caption_text = _caption_words(context.caption)
...
def _to_letter_index(val: str | None) -> int | None:
    if val is None:
        return None

    val = val.strip().upper()
    ru_letters = 'АБВГДЕЖЗИЙКЛМНОПРСТУФХЦЧШЩЫЭЮЯ'
    if val in ru_letters:
        return ru_letters.index(val) + 1
    if val in string.ascii_uppercase:
        return string.ascii_uppercase.index(val) + 1
    return None


def _is_latin_or_cyrillic_a(value: object) -> bool:
    return str(value) in {'A', 'А'}



def _caption_words(caption: FoundedCaption) -> list[str]:
    return [word.strip() for word in caption.text.split() if word]


def _table_caption_start_kind(caption_text: list[str], cutoff: float) -> str | None:
